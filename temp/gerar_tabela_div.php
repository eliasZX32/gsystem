<?php
	...
	$html_cadastro  = "<div class='container>";
		...
		$html_cadastro .= "<div class='row' id='table_{$table}'>";
			$html_cadastro .= "<div class='col-12'><h4 class='card-title'>{$table_caption}</h4></div>";
			$html_cadastro .= "<div class='col-12'><a onclick=\"button_new( '{$text_son}{$table_}' );\" class='btn btn-primary btn-sm' style='cursor:pointer'>Novo Registro</a></div>";	
		$html_cadastro .= "</div>\n";

		$html_cadastro .= "<div class='row' id='row_{$text_son}{$table_}_0' style='margin-left: 2px;'>\n";
			/* Carrega o cabeçalho do grid */	
			foreach( $struct as $field ){    
				if (array_key_exists("grid", $field)) {
					if( strtoupper($device) == "COMPUTER" ){
						if( strtoupper($field['grid']) == 'SHOW' || $field['grid'] == '' || strtoupper($field['grid']) == 'MOBILE'){
							$html_cadastro .= "	<div class='col-{$field['col']}' style='border: 1px solid #ddd;'><strong>{$field['label']}</strong></div>\n";
						}
					}else{
						if( strtoupper($field['grid']) == 'MOBILE' ){
							$html_cadastro .= "	<div class='col-{$field['col']}' style='border: 1px solid #ddd;'><strong>{$field['label']}</strong></div>\n";
			...
			$html_cadastro .= "	<div class='col-2' style='border: 1px solid #ddd;'><strong>Ação</strong></div>\n";
		$html_cadastro .= "</div>\n";
	    ...
	    */
		...
		{	
			$cont_linha++;
			$html_cadastro .= "<div class='row' id='row_{$text_son}{$table_}_{$cont_linha}' style='margin-left: 2px;'>\n";
			...
			{	
				...
				if( array_search( $col , $show_fields_grid) ){
					...
					//	}
	 				//	if ( $arr[$col] == "radio") {
					//		$html_cadastro .= "	<div class='col-12'>{$field['label']}</div>\n";
					...
					//	} 	
						...
						foreach( $struct as $field ){
							//var_dump($field);
							if( strtoupper($field['Field']) == strtoupper( $temp[1] ) ){
								if( strtoupper($device) == "COMPUTER" ){
									if( strtoupper($field['grid']) == 'SHOW' || $field['grid'] == '' || strtoupper($field['grid']) == 'MOBILE'){
										$html_cadastro .= "	<div class='col-{$field['col']}' style='border: 1px solid #ddd;'>{$val}</div>";
									}
								}else{
									if( strtoupper($field['grid']) == 'MOBILE' ){
										$html_cadastro .= "	<div class='col-{$field['col']}' style='border: 1px solid #ddd;'>{$val}</div>\n";
					 	...
					 	//$html_cadastro .= "	<div class='col-12' id={$col} name={$col} hidden>{$val}</div>\n";
					 	//echo $col . " ";
			...
			$html_cadastro .= "	<div class='col-1' style='border: 1px solid #ddd;' ><a onclick=\"button_edit({$cont_linha},'{$text_son}{$table}');\" class='btn btn-warning btn-sm' style='cursor:pointer' >Editar</a></div>";
			$html_cadastro .= "	<div class='col-1' style='border: 1px solid #ddd;' ><a onclick=\"button_delete({$cont_linha},'{$text_son}{$table}');\" class='btn btn-danger btn-sm' style='cursor:pointer'>Excluir</a></div>";
		    ...
		    //$html_cadastro .= "		<a onclick=\"button_delete({$cont_linha},'{$text_son}{$table}');\" class='btn btn-danger btn-sm' style='cursor:pointer'>Excluir</a>";
			//$html_cadastro .= "</td>";
